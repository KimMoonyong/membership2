<template>
    <v-card outlined>
        <v-card-title>
            Benefits # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="AgreementId" v-model="item.agreementId" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/benefits/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
